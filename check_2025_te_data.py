...
# Load the receiving_rushing data (2025 season stats)
csv_path = "data/processed/all_te_receiving_rushing.csv"

if os.path.exists(csv_path):
    df = pd.read_csv(csv_path)
    
    print("=" * 70)
    print("2025 TE SEASON DATA VERIFICATION")
    print("=" * 70)
    print(f"\nFile: {csv_path}")
    ...
    if 'Season' in df.columns:
        print(f"\nYears present: {sorted(df['Season'].unique())}")
