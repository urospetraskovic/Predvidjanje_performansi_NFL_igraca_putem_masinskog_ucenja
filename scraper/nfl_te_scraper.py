#!/usr/bin/env python3
"""
NFL TE Stats Scraper - Opera GX Version
...
Extracts: Receiving/Rushing, Advanced Receiving/Rushing, Defense/Fumbles, Snap Counts.

Usage:
    python nfl_te_scraper.py                    # Scrape all TEs in the list
    python nfl_te_scraper.py --player KelcTr00  # Scrape single player
    ...
    python nfl_te_scraper.py --list             # List scraped players
    python nfl_te_scraper.py --check            # Show which TEs are missing tables
"""
...
NFL_TES = [
    # ── Current/Recent Active TEs (2024-2025 Season) ─────────────────────
    ...
    # AFC SOUTH
    ('OkonCh00', 'Chigoziem Okonkwo'),    # Tennessee Titans (FIXED ID/name)
    ('EngrEv00', 'Evan Engram'),          # Jacksonville Jaguars
    ('SchuDa00', 'Dalton Schultz'),       # Houston Texans

    # AFC WEST
    ('KelcTr00', 'Travis Kelce'),         # Kansas City Chiefs
    ('DissWi00', 'Will Dissly'),          # Los Angeles Chargers
    ('MoreFo00', 'Foster Moreau'),        # New Orleans Saints (FIXED ID)
    ...
    ('WallDa01', 'Darren Waller'),        # New York Giants

    # Brock Bowers
    ('BoweBr00', 'Brock Bowers'),         # Las Vegas Raiders (FIXED ID)

    # ── Legacy / Hall of Fame / Veteran TEs ──────────────────────────────
...
def check_missing(input_dir='data/raw/te'):
    """Report which TEs are missing tables."""
    expected_files = ['receiving_rushing.csv', 'defense_fumbles.csv',
                      'advanced_receiving_rushing.csv', 'snap_counts.csv']

    player_folders = sorted([f for f in os.listdir(input_dir)
                            if os.path.isdir(os.path.join(input_dir, f))])

    print(f"\nChecking {len(player_folders)} TE folders for missing tables:\n")
...
def list_players(input_dir='data/raw/te'):
    """List all scraped players and their stats."""
    print(f"\nScraped TEs in {input_dir}:\n")
